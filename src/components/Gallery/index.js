...
class Gallery extends Component {
  state = {selectedObj: imagesList[0]}

  selectImage = id => {
    const selectedImageObj = imagesList.filter(each => each.id === id)
    this.setState({selectedObj: selectedImageObj[0]})
  }

  render() {
    const {selectedObj} = this.state
    const {imageUrl, imageAltText} = selectedObj
    return (
          ...
          <ul className="thumbnail-image-list">
            {imagesList.map(each => (
              <ThumbnailItem
                eachObj={each}
                key={each.id}
                isActive={each.id === selectedObj.id}
